  ...
  await panel.send({
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription("🛒 Köp premiumkonton")
  ...
  embeds: [
    new EmbedBuilder()
      ...
      .setDescription(
        ...
        `🎟 **Köp direkt**\n` +
        `Skapa en ticket här → <#${CONFIG.CHANNELS.PANEL}>\n\n` +

        `⭐ **Omdömen:** <#1452263084646338582>\n` +
        `✅ **Färdiga orders:** <#1452285768742600755>`
      )
      .setColor(CONFIG.BRAND.COLOR)
  ...
  try {
    ...
    if (interaction.isButton() && interaction.customId === "seller_approve") {
      ...
      return interaction.update({
        embeds: [
          new EmbedBuilder()
            .setTitle("💳 Betalning")
            .setDescription("Välj betalmetod")
        ],
        components: [
          new ActionRowBuilder().addComponents(
    ...
    if (interaction.isButton() && interaction.customId.startsWith("pay_")) {
      ...
      return interaction.update({
        embeds: [
          new EmbedBuilder()
            .setTitle("📸 Skicka betalning")
            .setDescription("Skicka screenshot efter betalning.")
        ],
        components: [
          new ActionRowBuilder().addComponents(
            new ButtonBuilder()
    ...
    if (interaction.isModalSubmit() && interaction.customId === "deliver_modal") {
      ...
      await user.send(
        ...
        ch.send("⚠️ Kundens DM är stängt.")
      );
